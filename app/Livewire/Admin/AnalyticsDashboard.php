...
class AnalyticsDashboard extends Component
{
    ...
    public function mount()
    {
        ...
        $this->totalClients = User::whereNotIn('role', ['admin', 'super_admin'])->count();
        ...
        $countPart = User::where('client_type', 'particulier')
            ->whereNotIn('role', ['admin', 'super_admin'])
            ->count();

        $countEnt = User::where('client_type', 'entreprise')
            ->whereNotIn('role', ['admin', 'super_admin'])
            ->count();

        $countTour = User::where('client_type', 'touriste')
            ->whereNotIn('role', ['admin', 'super_admin'])
            ->count();

        // On calcule le total sur la base de ces 3 chiffres uniquement pour garantir 100%
        $totalClients = $countPart + $countEnt + $countTour;

        if ($totalClients > 0) {
            $this->clientStats = [
                'particulier' => round(($countPart / $totalClients) * 100),
                'entreprise' => round(($countEnt / $totalClients) * 100),
                'touriste' => round(($countTour / $totalClients) * 100),
            ];
        } else {
